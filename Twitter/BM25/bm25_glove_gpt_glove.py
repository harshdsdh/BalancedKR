...
def get_recommended_queries(q_w, q_str):
    ...
    res_set = []
    for w in ans_list:
        sent = f'For original query {q_str}, frame a new query using {w}. Be as brief as possible'
...
def main():
    ...
    for row in set_queries:
        ...
        original_bias = add_to_excel(metrics, query_str, 'bm25_glove_gpt_glove')
        
        result = get_pareto_result(X,Y, original_bias, metrics)
        add_to_excel(result, query_str, 'paretoFront_bm25_glove_gpt_glove')
